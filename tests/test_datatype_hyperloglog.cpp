...
bool testHyperLogLogBasic() {
    ...
    bool added = storage.pfadd(key, elements);
    assert(added == true);
    
    uint64_t count = storage.pfcount(key);
    assert(count > 0);
    assert(count <= 3); // 由于HyperLogLog是概率性算法，结果可能不等于精确值，但应该接近
    
    // 测试添加重复元素
    added = storage.pfadd(key, {"element1"});
    assert(added == false); // 应该返回false，表示没有改变
    
    // 测试PFMERGE
    std::string key2 = "hll_test2";
    storage.pfadd(key2, {"element4", "element5"});
    
    bool merged = storage.pfmerge("hll_merged", {key, key2});
    assert(merged == true);
    
    uint64_t merged_count = storage.pfcount("hll_merged");
...
bool testHyperLogLogExpiration() {
    ...
    // 添加元素并设置过期时间
    storage.pfadd(key, {"element1"});
    storage.expire(key, 1);
